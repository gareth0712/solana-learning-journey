...
async fn success() {
  ...
  let transaction = Transaction::new_signed_with_payer(
    &[
      ...
      spl_token::instruction
        ::initialize_mint(&spl_token::id(), &mint.pubkey(), &payer.pubkey(), None, decimals)
        .unwrap(),
    ],
    Some(&payer.pubkey()),
    &[&payer, &mint],
  ...
  let transaction = Transaction::new_signed_with_payer(
    &[
      ...
      spl_token::instruction
        ::initialize_account(&spl_token::id(), &source.pubkey(), &mint.pubkey(), &authority_pubkey)
        .unwrap(),
    ],
    Some(&payer.pubkey()),
    &[&payer, &source],
  ...
  let transaction = Transaction::new_signed_with_payer(
    &[
      spl_token::instruction
        ::mint_to(&spl_token::id(), &mint.pubkey(), &source.pubkey(), &payer.pubkey(), &[], amount)
        .unwrap(),
    ],
    Some(&payer.pubkey()),
    &[&payer],
  ...
  let account = banks_client.get_account(destination.pubkey()).await.unwrap().unwrap();
