...
fn process_instruction(
  ...
  invoke_signed(
    ...
    &[user.clone(), user_derived_account.clone(), system_program_account.clone()],
    &[&[user.key.as_ref(), &[bump_seed]]]
  )?;

  let mut account_data = try_from_slice_unchecked::<UserStake>(
    &user_derived_account.data.borrow()
  ).unwrap();
