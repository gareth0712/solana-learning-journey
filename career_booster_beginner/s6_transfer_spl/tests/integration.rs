...
use ::{
  ...
  solana_program::{ instruction::{ AccountMeta, Instruction }, pubkey::Pubkey },
  solana_sdk::signer::keypair::Keypair,
  solana_sdk::system_program,
  solana_sdk::system_transaction,
  solana_sdk::{ signature::Signer, transaction::Transaction },
  solana_validator::test_validator::*,
  s6_transfer_spl::TransferInstruction,
};
...
fn test_validator_transaction() {
  ...
  let (test_validator, payer) = TestValidatorGenesis::default()
    .add_program("s6_transfer_spl", program_id)
    .start();
